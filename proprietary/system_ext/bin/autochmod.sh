    ...
    while [ true ];do
        if [ ! -d /cache/unboot_$i ];then
            ...
            if [ "$is_folder_empty" = "" ];then
                ...
                mv ${DATA_DEBUGGING_PATH}/cache ${DATA_DEBUGGING_PATH}/unboot_$i
            fi
            break
        else
            i=`$XKIT expr $i + 1`
...
function initcache(){
    ...
    if [[ x"${panicenable}" = x"true" ]] || [[ x"${camerapanic}" = x"true" ]] && [[ x"${boot_completed}" != x"1" ]]; then
        ...
        if [[ "$is_admin_empty" != "0" ]];then
            traceTransferState "backup_unboot_log"
            backup_unboot_log
        fi
...
function initOplusLog(){
    ...
    if [ "${PANICE_NABLE}" = "true" ] || [ x"${CAMERA_PANIC_ENABLE}" = x"true" ]; then
        ...
        if [ x"${decrypt}" != x"true" ]; then
            ...
            mv ${DATA_DEBUGGING_PATH}/unboot_* ${DATA_LOG_DEBUG_PATH}/
        fi
...
function transferSystemInfoLog(){
...
function transfer_m_systeminfo(){
    traceTransferState "start transfer_systeminfo"

    #init module path
    MODULE_TARGET_PATH=${TARGET_PATH}/systeminfo

    DUMP_SYSTEM_LOG=${DATA_DEBUGGING_PATH}/SI_stop
    # before mv systeminfo, wait for dump info done
    count=0
    while [[ ${count} -le 30 ]] && [[ ! -f ${DUMP_SYSTEM_LOG}/finish_system ]];do
        traceTransferState "waiting dump system info: ${count}" "w"
        count=$((count + 1))
        sleep 1
    done

    checkNumberSizeAndMove ${DATA_DEBUGGING_PATH}/SI_stop "${MODULE_TARGET_PATH}"
}
function transfer_m_qxdm() {
    ...
    checkNumberSizeAndCopy ${DATA_DEBUGGING_PATH}/diag_logs "${MODULE_TARGET_PATH}"
}
...
function logcatftm(){
    ...
    FTM_RESERVE_PATH=/mnt/vendor/opporeserve/ftm_admin
...
function getSystemStatus() {
    ...
    if [[ x${boot_completed} == x"1" ]]; then
        outputPath="${DATA_DEBUGGING_PATH}/SI_stop"
